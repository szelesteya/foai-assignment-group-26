...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def minimax_root(
        self, state: GameState, depth: int, moves: List[Move], opp_reach: Set[Tuple[int, int]]
    ) -> Optional[Move]:
        best_val = -math.inf
        best_move = None
        alpha = -math.inf
    ...
    def minimax(
        ...
        beta: float,
        is_maximizing: bool,
        initial_opp_reach: Set[Tuple[int, int]],
    ) -> float:
        if depth == 0:
    ...
    def find_single_holes(self, board: SudokuBoard) -> List[Tuple[int, int]]:
        ...
        for r in range(N):
            check_coords([(r, c) for c in range(N)])
        for c in range(N):
            check_coords([(r, c) for r in range(N)])
    ...
    def calculate_points_gained(self, board: SudokuBoard, r: int, c: int) -> int:
        regions = 0
        N = board.N
        if sum(1 for i in range(N) if board.get((r, i)) != 0) == N:
            regions += 1
        if sum(1 for i in range(N) if board.get((i, c)) != 0) == N:
            regions += 1
        m, n = board.m, board.n
        sr, sc = (r // m) * m, (c // n) * n
        if sum(1 for i in range(m) for j in range(n) if board.get((sr + i, sc + j)) != 0) == m * n:
            regions += 1
