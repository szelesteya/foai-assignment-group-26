...
def play_match(
    ...
    for i in range(1, count + 1):
        ...
        result = play_game(
            board_file, first, second, calculation_time, verbose, warmup and i == 1
        )

        result_line = (
            f"{first} - {second} {print_score(result[0])}-{print_score(result[1])}\n"
        )
...
def main():
    ...
    cmdline_parser = argparse.ArgumentParser(
        description="Play a match between two sudoku players."
    )
    ...
    cmdline_parser.add_argument(
        "--count", type=int, default=6, help="The number of games (default: 6)"
    )
    cmdline_parser.add_argument(
        "--board",
        type=str,
        default="boards/empty-2x2.txt",
        help="The text file containing the start position (default: boards/empty-2x2.txt)",
    )
    cmdline_parser.add_argument(
        "--time",
        type=float,
        default=3.0,
        help="The time (in seconds) for computing a move (default: 3.0)",
    )
    cmdline_parser.add_argument(
        "--verbose", help="Give verbose output", action="store_true"
    )
    cmdline_parser.add_argument(
        "--warm-up",
